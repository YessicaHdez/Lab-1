...
public class GreetingRestControllerTest {
    ...
    public void givenARequest_WhenEmptyName_thenGoodbye() throws Exception {
        this.mockMvc.perform(get("/goodbye"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("content", equalTo("Goodbye, Cersie!")));
    }
    
    @Test
    public void givenAGoodbye_whenProvidedName_thenGoodbyeName() throws Exception {
        this.mockMvc.perform(get("/goodbye?name=Sansa"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("content", equalTo("Goodbye, Sansa!")));
    }
